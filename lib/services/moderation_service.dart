...
class ModerationItem {
  ...
  factory ModerationItem.fromJson(Map<String, dynamic> j) => ModerationItem(
        ...
        status: (j['status'] as String? ?? 'PENDING_REVIEW').toLowerCase().replaceAll('_review', '').replaceAll('ai_flagged', 'flagged'),
      );
}
...
class ModerationService {
  static Future<List<ModerationItem>> getQueue() async {
    ...
    return list.map((e) => ModerationItem.fromJson(e as Map<String, dynamic>)).toList();
  }
  ...
  static Future<void> review(int reelId, {required bool approve, String note = ''}) async {
    await ApiClient.instance.post('/api/admin/moderation/$reelId/review', data: {
