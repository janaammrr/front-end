...
class _ModerationPanelScreenState extends State<ModerationPanelScreen>
  ...
  List<ModerationItem> _byStatus(String status) =>
      _all.where((i) => i.status == status).toList();
                  ...
                  child: TabBarView(
                    ...
                    children: [
                      _ModerationList(
                        ...
                        onApprove: (item) => _review(item, true),
                        onReject: (item) => _review(item, false),
                        showActions: true,
                      ),
                      _ModerationList(
                        items: approved,
...
class _ModerationList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty) {
      return Center(
        child: Text('Nothing here', style: TextStyle(color: AppColors.text3)),
      );
    }
    return ListView.separated(
      ...
      itemBuilder: (_, i) => _ModerationCard(
        item: items[i],
        processing: processingIds.contains(items[i].reelId),
        onApprove: onApprove,
        onReject: onReject,
                    ...
                    Row(
                      children: [
                        ...
                        if (item.aiConfidenceScore > 0) ...[
                          ...
                          Text(
                            '${(item.aiConfidenceScore * 100).toStringAsFixed(0)}% confidence',
                            style: TextStyle(
                              color: AppColors.text3,
                              fontSize: 11,
                            ),
