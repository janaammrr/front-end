...
class _CommunitiesScreenState extends State<CommunitiesScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) setState(() { _all = communities; _loading = false; });
    } catch (e) {
  ...
  List<CommunityModel> get _filtered {
    ...
    return list.where((c) => c.name.toLowerCase().contains(q) || c.description.toLowerCase().contains(q)).toList();
  }
          ...
          child: BackdropFilter(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  Center(child: Container(width: 40, height: 4, margin: const EdgeInsets.only(bottom: 20), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(999)))),
                  const Text('Create Community', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: Colors.white)),
                  const SizedBox(height: 16),
                  _StyledTextField(controller: nameCtrl, hint: 'Community name'),
                  const SizedBox(height: 12),
                  _StyledTextField(controller: descCtrl, hint: 'Description', maxLines: 3),
                  const SizedBox(height: 20),
                  SizedBox(
                    width: double.infinity,
                    child: DecoratedBox(
                      decoration: BoxDecoration(
                        gradient: const LinearGradient(colors: [Color(0xFFFF7A18), Color(0xFFFFB073)]),
                        borderRadius: BorderRadius.circular(14),
                        boxShadow: [BoxShadow(color: const Color(0xFFFF7A18).withValues(alpha: 0.35), blurRadius: 14, offset: const Offset(0, 4))],
                      ),
                      child: ElevatedButton(
                        ...
                        onPressed: creating ? null : () async {
                          ...
                          try {
                            final newComm = await CommunityService.create(nameCtrl.text.trim(), descCtrl.text.trim());
                            if (ctx.mounted) Navigator.of(ctx).pop();
                            if (mounted) setState(() => _all = [newComm, ..._all]);
                          } catch (e) {
                            setSheet(() => creating = false);
                            if (ctx.mounted) {
                              ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content: Text('Error: $e'), backgroundColor: const Color(0xFFEF4444)));
                            }
                          }
                        },
                        child: creating
                            ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                            : const Text('Create', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
                      ),
                    ),
                  ),
                ],
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Container(
              width: 88, height: 88,
              decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0xFFFF7A18).withValues(alpha: 0.1), border: Border.all(color: const Color(0xFFFF7A18).withValues(alpha: 0.3))),
              child: Icon(isFollowing ? Icons.groups_outlined : Icons.search_off_rounded, color: const Color(0xFFFF7A18), size: 40),
            ),
            const SizedBox(height: 20),
            Text(isFollowing ? 'No communities yet' : 'No results found', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            Text(
              isFollowing ? 'Join communities from the For You tab.' : 'Try a different search term.',
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(color: const Color(0xFFFF7A18).withValues(alpha: 0.35), blurRadius: 12, offset: const Offset(0, 4))],
                  ),
                  child: const Text('Discover communities', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 14)),
                ),
              ),
            ],
          ],
  ...
  SnackBar _snackBar(String msg) => SnackBar(
        content: Row(children: [const Icon(Icons.check_circle_rounded, color: Colors.white, size: 18), const SizedBox(width: 10), Text(msg)]),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: [
          Container(decoration: const BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0xFF0F0A00), Color(0xFF09090B)]))),
          SafeArea(
            bottom: false,
            child: Column(
              children: [
                ...
                ClipRect(
                  child: BackdropFilter(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.04), border: Border(bottom: BorderSide(color: Colors.white.withValues(alpha: 0.08)))),
                      child: Row(
                        children: [
                          ShaderMask(
                            shaderCallback: (r) => const LinearGradient(colors: [Color(0xFFFF7A18), Color(0xFFB83280)]).createShader(r),
                            child: const Text('Flame', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w800, letterSpacing: -0.5)),
                          ),
                          const Spacer(),
                          Container(
                            ...
                            child: Row(
                              children: [
                                GestureDetector(onTap: () => setState(() => _selectedTab = 0), child: _PillTab(label: 'For You', active: _selectedTab == 0)),
                                GestureDetector(onTap: () => setState(() => _selectedTab = 1), child: _PillTab(label: 'Joined', active: _selectedTab == 1)),
                              ],
                            ),
                          ),
                          const Spacer(),
                          Stack(
                            ...
                            children: [
                              ...
                              Positioned(top: 6, right: 6, child: Container(width: 16, height: 16, decoration: const BoxDecoration(color: Color(0xFFFF7A18), shape: BoxShape.circle), child: const Center(child: Text('1', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w700))))),
                            ],
                          ),
                          IconButton(
                            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MessagingScreen())),
                ...
                Padding(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: Color(0xFFFF7A18), width: 1.5)),
                    ),
                  ),
                ),
...
class _CommunityCard extends StatelessWidget {
  ...
  String get _logoText {
    ...
    return community.name.substring(0, community.name.length.clamp(0, 2)).toUpperCase();
  }
              ...
              children: [
                Row(
                  children: [
                    Container(
                      width: 56, height: 56,
                      decoration: BoxDecoration(gradient: LinearGradient(colors: _gradient, begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.white.withValues(alpha: 0.15))),
                      child: Center(child: Text(_logoText, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 18, letterSpacing: -0.5))),
                    ),
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(community.name, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w700, color: Colors.white)),
                        ],
                      ),
                    ),
                  ],
                ...
                if (community.description.isNotEmpty) ...[
                  ...
                  Text(community.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 13, color: Colors.white.withValues(alpha: 0.6), height: 1.4)),
                ],
                const SizedBox(height: 12),
                ...
                Row(
                  children: [
                    CircleAvatar(radius: 14, backgroundColor: const Color(0xFFFF7A18).withValues(alpha: 0.18), child: Text(community.admin.initials.isNotEmpty ? community.admin.initials[0] : '?', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFFFF7A18)))),
                ...
                Row(
                  children: [
                    ...
                    _SmallActionButton(icon: Icons.ios_share_outlined, color: Colors.white.withValues(alpha: 0.4), onTap: onShare),
                  ],
                ),
              ],
            ),
  ...
  Widget _buildJoinButton() {
    if (isLoading) {
      return Container(
        ...
        decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.07), borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.white.withValues(alpha: 0.15))),
        child: const Center(child: SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFFFF7A18)))),
      );
    }
      ...
      return GestureDetector(
        ...
        child: Container(
          ...
          child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            SizedBox(width: 6),
            Text('Joined', style: TextStyle(color: Color(0xFFFFB073), fontWeight: FontWeight.w600, fontSize: 15)),
          ]),
        ),
      );
    }
    ...
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(vertical: 13),
        decoration: BoxDecoration(
          gradient: const LinearGradient(colors: [Color(0xFFFF7A18), Color(0xFFFFB073)], begin: Alignment.centerLeft, end: Alignment.centerRight),
          borderRadius: BorderRadius.circular(14),
          boxShadow: [BoxShadow(color: const Color(0xFFFF7A18).withValues(alpha: 0.35), blurRadius: 10, offset: const Offset(0, 4))],
        ),
        child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.person_add_outlined, color: Colors.white, size: 18),
          SizedBox(width: 6),
          Text('Join', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 15)),
        ]),
      ),
    );
  }
...
class _PillTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        gradient: active ? const LinearGradient(colors: [Color(0xFFFF7A18), Color(0xFFFFB073)]) : null,
